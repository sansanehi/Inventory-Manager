...
import { FaDownload, FaChartBar, FaChartLine, FaChartPie, FaBox, FaUsers, FaDollarSign } from 'react-icons/fa';
import {
  BarChart,
  Bar,
  LineChart,
...
import { productsStore, customersStore, ordersStore, transactionsStore } from '../../store/localStore';
...
const Reports = () => {
  ...
  const generateReports = () => {
    ...
    try {
      ...
      // Validate data
      if (!Array.isArray(orders) || !Array.isArray(products) || !Array.isArray(customers) || !Array.isArray(transactions)) {
  ...
  const generateProductReport = (products, orders) => {
    ...
    return products.map(product => {
      ...
      const totalSold = productOrders.reduce((sum, order) => {
        const item = order.items.find(item => item && item.productId === product.id);
        return sum + (Number(item?.quantity) || 0);
      }, 0);

      return {
        name: product.name || `${product.brand || ''} - ${product.model || 'Unknown Product'}`,
        totalSold: Number(totalSold),
        currentStock: Number(product.quantity) || 0
      };
    }).filter(Boolean).sort((a, b) => b.totalSold - a.totalSold);
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-bold mt-1">
                    ${reportData.sales.reduce((sum, item) => sum + item.amount, 0).toFixed(2)}
                  </h3>
                </div>
